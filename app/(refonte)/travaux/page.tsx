...
/* Index portfolio de la refonte.
 *
 * Chaque catégorie est une grande affiche : sa couverture en plein cadre, son
 * titre en serif, son numéro fantôme et son décompte, cliquable vers la page
 * détail (séries + médias). La grille est en `auto-fit` : une seule catégorie
 * occupe alors toute la largeur — l'index reste plein et affirmé même avec peu
 * de contenu, au lieu de la liste texte qui paraissait vide. */

export default async function SeriesIndexPage() {
  const [cats, content] = await Promise.all([getCategories(), getSiteContent()]);

  // Résumé chiffré, calculé (jamais écrit en dur) : donne du poids à l'en-tête.
            ...
            <li key={c.slug}>
              <Reveal as="div" delay={Math.min(i, 6) * 90}>
                ...
                >
                  ...
                  {/* Dégradé : sombre en bas (lisibilité du titre) et un voile
                      haut pour le numéro fantôme. */}
                  <span
                    aria-hidden
                    style={{
                      position: "absolute",
                  ...
                  {/* Numéro fantôme, grand, en haut à droite. */}
                  <span
                    aria-hidden
                    style={{
                      position: "absolute",
